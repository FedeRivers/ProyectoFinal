...
namespace ProyectoFinal._3_Persistencia
{
    public class PEstadistica
    {
        ...
        public ObtenerEstadisticasOut ObtenerCantidadesPorEspecie(ObtenerEstadisticasIn input)
        {
            ...
            using (var dataContext = new ModeloEstadisticaDataContext())
            {
                try
                {
                    var result = dataContext.ObtenerCantidadesPorEspecie(input.FechaDesde, input.FechaHasta);
                    if (result != null)
                    {
        ...
        public ObtenerEstadisticasOut ObtenerDevueltosYAlmacenados(ObtenerEstadisticasIn input)
        {
            var output = new ObtenerEstadisticasOut { Datos = new List<ItemValor>(), Status = new HttpStatusCodeResult(400) };
            using (var dataContext = new ModeloEstadisticaDataContext())
            {
                try
                {
                    var result = dataContext.ObtenerDevueltosYAlmacenados(input.FechaDesde, input.FechaHasta);
                    if (result != null)
                    {
                        foreach (var item in result)
                        {
